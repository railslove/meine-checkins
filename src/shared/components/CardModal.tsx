...
import Animated, {cond, multiply, not, or, set, useCode, Value} from 'react-native-reanimated';
...
const CardModal = ({children, visible, cardHeight = layoutUtil.height, almostTuckedIn = false, cardStyle}: Props) => {
  const animation = useValue(0);
  const heightValue = useMemo(() => multiply(-cardHeight, HEIGHT_OFFSET), [cardHeight]);
